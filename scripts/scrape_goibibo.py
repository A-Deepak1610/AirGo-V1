"""
CLI Runner for Goibibo Scraper.
Usage:
    python scripts/scrape_goibibo.py --route BOM-DEL --horizons 1,7,15,30,45
    python scripts/scrape_goibibo.py --route BOM-DEL --horizons 1 --visible
    python scripts/scrape_goibibo.py --route BOM-DEL --horizons 45 --visible --pause 20
"""
...
def main():
    parser = argparse.ArgumentParser(description="AirGo Goibibo Live Airfare Scraper with Chrome CDP & Anti-Bot Bypass")
    ...
    parser.add_argument("--visible", action="store_true", help="Run visible Chrome browser instead of headless/offscreen")
